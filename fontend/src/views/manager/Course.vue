  ...
  <div>
    <div class="card" style="margin-bottom: 10px">
      <el-input @input="load" style="width: 260px" v-model="data.name" placeholder="请输入要查询的课程名称"
                :prefix-icon="Search"/>
      <el-input @input="load" style="margin-left:10px; width: 260px" v-model="data.number"
                placeholder="请输入课程编号" :prefix-icon="Search"/>
      <el-input @input="load" style="margin-left:10px; width: 260px" v-model="data.teacher"
                placeholder="请输入任课老师" :prefix-icon="Search"/>
...
const load = () => {
  request.get('/course/selectPage', {
    ...
    if (res.code === '200') {
      data.tableData = res.data?.pageList || []
